...
Then using these probabilities we define Cross Entropy Loss ($\text{CE}$) on a batch of size $N$ for true labels $	\mathbf{y} = (y\_1, \ldots, y\_N)$ as,

$$
CE = -\frac{1}{N}\sum\_{i=1}^N \left[y\_i\cdot \log(p\_{i,0}) +  (1-y\_i)\log(p\_{i,1}) \right],
$$
